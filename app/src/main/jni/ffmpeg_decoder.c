...
#define LOGI(format, ...)  __android_log_print(ANDROID_LOG_ERROR,  "yijun", format, ##__VA_ARGS__)
#else
...
JNIEXPORT jint JNICALL Java_com_example_ffmpeg_DecodeActivity_decode
    ...
    if (avformat_open_input(&pFormatCtx, input_str, NULL, NULL) != 0) {
        LOGE("Couldn't open input stream.file:%s\n",input_str);
        return -1;
    }
    //获取压缩流
...
//Java_com_example_ffmpeg_AudioActivity_audioPlayer(JNIEnv *env, jclass type, jstring url_) {
