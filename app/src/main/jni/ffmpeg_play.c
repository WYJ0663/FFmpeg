...
* 初始化封装格式初始化
*/
int init_input_format_ctx(const char *input_cstr, Player *player) {
...
void jni_audio_prepare(JNIEnv *env, jobject jthiz, Player *player) {
    //JNI begin------------------
    jclass cls = (*env)->FindClass(env, "com/example/ffmpeg/AudioUtil");
    //jmethodID
    ...
    jobject audio_track = (*env)->CallObjectMethod(env, audioutil_obj, create_audio_track_mid, player->out_sample_rate,
...
//    avcodec_decode_video2(codec_ctx, pFrame, &got_frame, packet);
    ...
    int re = avcodec_send_packet(codec_ctx, packet);
    if (re != 0) {
        return;
    }
...
void *decode_data(void *arg) {
    ...
    while (&player->isPlay) {
        ...
        if (stream_index == player->audio_stream_index) {
...
//音视频同步播放
//视频播放
JNIEXPORT void JNICALL Java_com_example_ffmpeg_PlayActivity_play
    ...
    //初始化封装格式上下文
    if (!init_input_format_ctx(file_name, player)) {
        return;
    }
    //视频索引
